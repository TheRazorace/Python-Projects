...
print("Αλγόριθμος BruteForce1 --> O(n^3)")
...
def bf1(arr):
    ...
    for i in range (n):               #Πρώτο σκανάρισμα της λίστας
        for j in range(i+1, n+1):     #Δεύτερο σκανάρισμα της λίστας
            s=0
            for k in range(i,j):      #Τρίτη επάληψη που υπολογίζει υποαθροίσματα
                s += arr[k]
                if s > m[0]:
...
m = bf1(arr)                            #Υπολογισμός μέγιστου υποαθροίσματος πρώτου πίνακα
t1 = time.time()                        #Τέλος πρώτης χρονομέτρησης, αρχή δεύτερης χρονομέτρησησς                       
d = bf1(double_arr)                     #Υπολογισμός μέγιστου υποαθροίσματος δεύτερου πίνακα
