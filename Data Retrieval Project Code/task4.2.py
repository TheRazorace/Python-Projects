...
def FindMovies(es):
    ...
    if(total_results>0):
        print("Εύρεση σκορ χρήστη, σκορ συστάδας, σκορ πρόβλεψης και μέσου σκορ...")
        ...
        ratings, new_ratings3, new_ratings4 = CreateRatingsratings(movie_ids)
        
        #Εύρεση σκορ χρήστη (αν υπάρχει), σκορ συστάδας, σκορ πρόβλεψης και μέσο σκορ
        user_score, avg_score, cluster_score, embedding_score = RatingBasedScores(total_results,
                                movie_ids, ratings, user_in, new_ratings3, new_ratings4)
         
        #Υπολογισμός συνολικού σκορ
        total_score = TotalScore(total_results, elastic_score, user_score,
                                 avg_score, cluster_score, embedding_score) 
...
def CreateRatingsratings(movie_ids):
    ...
    try:
        ...
        new_ratings4 = pd.read_csv('new_ratings4.csv')
    except IOError:
    ...
    new_ratings4 = new_ratings4.loc[new_ratings4['movieId'].isin(movie_ids)]
    new_ratings4 = new_ratings4[['userId','movieId','rating']].reset_index()
    
    return ratings, new_ratings3, new_ratings4
    

#Συνάρτηση εύρεσης σκορ χρήστη (αν υπάρχει) και μέσο σκορ
def RatingBasedScores(total_results, movie_ids, ratings, user_in, new_ratings3, new_ratings4):
    
    #Πίνακες αποθήκευσης σκορ
    user_score = [0]*total_results
    avg_score = [0]*total_results
    cluster_score = [0]*total_results
    embedding_score = [0]*total_results
    ...
    for movie_id in movie_ids:
        ...
        #Αποθήκευση πρόβλεψη βαθμολογίας χρήστη για την ταινία
        embedding_score[i] = new_ratings4.loc[(new_ratings4["movieId"] == int(movie_id))
                            & (new_ratings4['userId'] == int(user_in))]["rating"].to_numpy()[0]
        
        i = i + 1
        
    return user_score, avg_score, cluster_score, embedding_score
    
#Συνάρτηση υπολογισμού συνολικού score
def TotalScore(total_results, elastic_score, user_score, avg_score, cluster_score, embedding_score):
    
    total_score = [0]*total_results
    #Βάρη score elasticsearch, χρήστη, μέσου score, score συστάδας και score πρόβλεψης αντίστοιχα
    w1 = 1
    w2 = 2
    w3 = 1
    w4 = 1.5
    ...
    for i in range (0,total_results):
        ...
        #... και το score της πρόβλεψης
            ...
            score = w1*elastic_score[i] + w3*avg_score[i] + w4*cluster_score[i] + w5*embedding_score[i]
            total_score[i] = score/4
